...
from align.deploy.helper import nms, adjust_input, generate_bbox, detect_first_stage_warpper
# from tensormsa_insightface.src.common.utils import make_total_boxes
#import tensormsa_insightface.src.common.utils as utils
...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        self.make_box_size = [5, 8] # [5, 8, 3, 2 ....]
    ...
    def make_total_boxes(self, total_boxes, width, height, wcnt):
        start_w = 0
        start_h = 0
        split_w = round(width / wcnt)
        split_h = round(height / (wcnt - 1))
        for j in range(wcnt - 1):
            for i in range(wcnt):
                add_boxes = np.array([[start_w, start_h, start_w + split_w, start_h + split_h, 0.9]], dtype=np.float32)
                start_w += split_w
                total_boxes = np.concatenate((total_boxes, add_boxes))
            start_w = 0
            start_h += split_h
        return total_boxes
    ...
    def detect_face(self, img):
        ...
        if self.total_boxes is None:
            total_boxes = np.array([[0.0, 0.0, img.shape[1], img.shape[0], 0.9]], dtype=np.float32)
            for box_size in self.make_box_size:
                total_boxes = self.make_total_boxes(total_boxes, img.shape[1], img.shape[0], box_size)
        else:
